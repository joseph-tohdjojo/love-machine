...
function _symlinkGitCompletion() {
  const fileToPointTo = path.resolve(__dirname, '..', '..', 'dotfiles', 'git', '.git-completion.bash')
...
function _symlinkGitExcludes() {
  const fileToPointTo = path.resolve(__dirname, '..', '..', 'dotfiles', 'git', '.gitexcludes')
  const target = `${HOMEDIR}/.gitexcludes`
  return symlink(fileToPointTo, target)
}

function _symlinkGitIgnore() {
  const fileToPointTo = path.resolve(__dirname, '..', '..', 'dotfiles', 'git', '.gitignore')
